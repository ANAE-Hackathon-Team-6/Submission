...
from get_most_similar_activities.from_submission_form import similar_activities_from_submission_form
...
pinecone_api_key = "pcsk_34KbSm_2WQhegrRfgD8uQjoUw8VNgjXhfgWxqA7KLMGZaqNjCB5XiDvjrdMkgasYhrWtZD"
...
convbot = ConversationalAI(pinecone_api_key, gemini_api_key, real_activities_path=real_activities_path)
...
def chatbot():
    ...
    response = convbot.conversational_ai(data['Query'])
...
searchbot = SearchAI(pinecone_api_key, gemini_api_key, real_activities_path=real_activities_path)
...
def noai():
    ...
    searchbot = Search(pinecone_api_key, gemini_api_key, real_activities_path=real_activities_path)
    response = searchbot.search_activity(data['Query'])
